...
C     to take a 1D cut from a 2D data set of pixels with (vx,vy) coordinates 
C     take only pixels with coordinate vy_min<=vy<=vy_max 
...
C declare local variables to deal with pointers to variables passed by/to MATLAB
      integer vx_pr, vy_pr, pixel_int_pr, pixel_err_pr, grid_pr
...
C declare local operating variables of the interface funnction
      ...
      double precision grid(5), vx_min, vx_max, bin_vx, 
     c    vy_min, vy_max, vvy, eps
  
C     Check for proper number of MATLAB input and output arguments 
      if (nrhs .ne. 5) then
         call mexErrMsgTxt
     c ('Five inputs (vx,vy,pixel_int,pixel_err,grid) required.')
      elseif (nlhs .ne. 6) then
         call mexErrMsgTxt
     c  ('Six outputs (x,intensity,err_int,perm,number_pix,vy) 
     c required.')
      end if
...
C     Get vx,vy,pixel_int,pixel_err and grid parameters
...
C     Create matrices for the return arguments (operating workspace for the subroutine cut2d_df)
...
C     Call the computational subroutine cut2d_df
      call cut2d_df(%val(vx_pr),%val(vy_pr),
     ...
     c     vvy,%val(temp1),%val(temp2),%val(temp3))
...
C     n = total number of bins	
     ...
     c     pixel_index3(*), bin_index(n), cumm_npixels(n), final_npixel
      double precision vx(*), vy(*), pixel_int(*), pixel_err(*), 
     c                 vx_min, vx_max, bin_vx, eps, vy_min, vy_max, 
