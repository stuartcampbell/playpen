...
C ===       energy_bin(i)=pixels(2,i) !  energy bin (meV) per pixel
            energy_centre=pixels(1,i)+(rand(seed)-0.5d0)*pixels(2,i)	! [meV]
...
C ===============================================================================
      ...
      real ran2
      randn=
     c sqrt(-2.0d0*log(ran2(seed)))*cos(8.0d0*atan(1.0d0)*ran2(seed))
      return
      end
...
130   CGAMMA = CDEXP(SUM)
      RETURN
900   FORMAT(1X,2E14.7,10X,'ARGUMENT OF GAMMA FUNCTION IS TOO CLOSE', 
     c' TO A POLE')
     ...
     c/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
      ...
      ! ====================================================================
         ...
         if (.not. read_tables) then
            ...
            filename='/matlab72/mprogs/mslice/sqw_1matr_in.dat'
            open(unit=1,file=filename, status='unknown')
            call mexPrintf('Opening file '//filename);
            filename='/matlab72/mprogs/mslice/sqw_1matr_out.dat'
            open(unit=2,file=filename, status='unknown')
            call mexPrintf('Opening file '//filename);
            filename='/usr/local/matlab72/mprogs/mslice/sqw_2matr.dat'
