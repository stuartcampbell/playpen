...
%
% Purpose: plot interface for a IXTpolygon objects
%
...
% inputs: 
%       w -         IXTpolygon object
%       format -    type of graph to plot, 
%       varargin -  optional settings for graph
%
% outputs: 
%
% figure handle, axes handle, plot handle
%
...
switch format
    ...
    if separate_flag == IXG_ST_STDVALUES.true
        for i = 1:len
            ...
            [figureHandle_ axesHandle_ plotHandle_] = ixf_gen_interface('iname','plot_interface','fname','patch',w(i),'format',format,'name',IXG_ST_STDVALUES.area_name,'tag',IXG_ST_STDVALUES.twod,'title',title,...
        ...
        counter_value = IXG_ST_STDVALUES.counter_reset;
            ...
            [figureHandle_ axesHandle_ plotHandle_] = ixf_gen_interface('iname','plot_interface','fname','patch',w,'format',format,'name',IXG_ST_STDVALUES.area_name,'tag',IXG_ST_STDVALUES.twod,'title',title,...
            'xlabel',xlab,'ylabel',ylab,'zlabel',zlab,'marker','none','counter', counter_value,varargin{:});
    end
        colorbar;
    ...
    case 'surface'
       ...
       error('Can not surface plot an IXTpolygons object.')
end