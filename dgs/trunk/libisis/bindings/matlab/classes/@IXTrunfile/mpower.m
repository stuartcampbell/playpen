function runfile = mpower(a,b)
%--- Help for IXTrunfile/mpower.m---
% call syntax: a+b or power(a,b)
%
...
if iscell(b) && len_a == len_b           % need to use cell syntax if either is a cell.
        ...
        for i = 1:length(a)                  % they must be equal length though
            runfile(i) = power_runfile(a(i),b{i});
        end
...
%--------function deals with numbers and datasets only------------
   ...
   function runfile = power_runfile(a, b)
...
if len_a == len_b 
    ...
    if isa(b,'IXTdataset_1d')
        error('power is not defined for IXTdataset_1d with IXTrunfile operations')
%       runfile = libisisexc('IXTrunfile','dataset_1d_power',IXTrunfile,a,b);
    else
        runfile = libisisexc('IXTrunfile','power',IXTrunfile,a,b);
    end
 
 elseif  (len_a == 1 && isnumeric(b)) || (len_b == 1 && isnumeric(a))    % single dataset, array of numbers 
        error('power is not defined for IXTrunfile with an array of numbers')
%        runfile = libisisexc('IXTrunfile','array_power',IXTrunfile,a,b);

elseif isa(b,'IXTdataset_1d')        % looking at array of d1d added to the d2d or single d1d added to array d2d
        error('power is not defined for IXTrunfile with an array of IXTdataset_1d')
%   len_d2d = length(a);
%    len_d1d = length(b);
%    
%     for i = 1:len_d2d
