function runfile = minus(a,b)
%--- Help for IXTrunfile/minus.m---
% call syntax: a+b or plus(a,b)
%
% subtracts an 1XTrunfile object from another 1XTrunfile
...
if iscell(b) && len_a == len_b           % need to use cell syntax if either is a cell.
        ...
        for i = 1:length(a)                  % they must be equal length though
            runfile(i) = minus_runfile(a(i),b{i});
        end
...
%--------function deals with numbers and datasets only------------
   ...
   function runfile = minus_runfile(a, b)
...
if len_a == len_b 
    ...
    if isa(b,'IXTdataset_1d')
        error('minus is not defined for IXTdataset_1d with IXTrunfile operations')
%       runfile = libisisexc('IXTrunfile','dataset_1d_plus',IXTrunfile,a,b);
    else
        runfile = libisisexc('IXTrunfile','minus',IXTrunfile,a,b);
    end
 
 elseif  (len_a == 1 && isnumeric(b)) || (len_b == 1 && isnumeric(a))    % single dataset, array of numbers 
        error('minus is not defined for IXTrunfile with an array of numbers')
...
elseif isa(b,'IXTdataset_1d')        % looking at array of d1d added to the d2d or single d1d added to array d2d
        error('minus is not defined for IXTrunfile with an array of IXTdataset_1d')
%   len_d2d = length(a);
%    len_d1d = length(b);
%    
%     for i = 1:len_d2d
