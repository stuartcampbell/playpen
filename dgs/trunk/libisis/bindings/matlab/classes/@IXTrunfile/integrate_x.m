function runfile = integrate_x(a,xmin,xmax)
% runfile = integrate_x(runfile,xmin,xmax)
%
% integrates all the detector data of an IXTrunfile along the x-axis between xmin and xmax to produce
...
% output will be
%
% runfile(i) = integrate_x(runfile(i),xmin(i),xmax(i))
%
...
if   len_a == len_xmin && len_xmin == len_xmax       % all array or all single
    ...
    for i = 1:len_a
        runfile(i) = libisisexc('IXTrunfile','integrate_x',IXTrunfile,a(i),[xmin(i) xmax(i)]);
    end

elseif len_a > 1 && len_xmin == 1 && len_xmax == 1   % only a is array

    for i = 1:len_a
        runfile(i) = libisisexc('IXTrunfile','integrate_x',IXTrunfile,a(i),[xmin,xmax]);
    end
