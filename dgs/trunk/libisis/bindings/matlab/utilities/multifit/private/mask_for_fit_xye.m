...
if ~isempty(x)
    ...
    elseif ~all(ok_xvals(mask))
        mess='Points with at least one undefined (infinite or NaN) coordinate value have been removed';
...
elseif ~all(ok_yvals(mask))
    mess='Points with undefined (infinite or NaN) data values have been removed from fit';
    if ~return_with_errors, disp(mess), else imess=imess+1; mess_cell{imess}=mess; end
end
...
elseif ~all(ok_ebars(mask))
    mess='Points with zero, negative or undefined (infinite or NaN) error bars have been removed from fit';
    if ~return_with_errors, disp(mess), else imess=imess+1; mess_cell{imess}=mess; end
end
