...
arglist = struct('fitcontrolparameters',[0.0001 30 0.0001],...
                 'list',0,'keep',[],'remove',[],'mask',[],'selected',0,'parsefunc_',0);
flags = {'selected','parsefunc_'};
...
for i=1:numel(w)
    ...
    if xye(i)    % xye triple
        ...
        if nodata && any(msk{i}(:)), nodata=false; end    % For speed, only test if data present if not already some data 
...
np=p_info.np;
nbp=p_info.nbp;
npfree=p_info.npfree;

if ~nodata  % not having any data at all is not uncommon, so catch this
    ...
    % Evaluate the functions at the fitted parameter values
    if options.selected
    ...
    display_mess({'*************************************************',...
                  '  No data left to fit - filling output with NaN',...
                  '*************************************************'})
    fitdata.p=NaN(1,np);
    fitdata.sig=NaN(1,np);
    if bkd
        fitdata.bp=reshape(mat2cell(NaN(1,sum(nbp(:))),1,nbp(:)'),size(nbp));
        fitdata.bsig=reshape(mat2cell(NaN(1,sum(nbp(:))),1,nbp(:)'),size(nbp));
    end
    fitdata.corr=NaN(npfree);
    fitdata.chisq=NaN;
    % Return the data as the input data
    wout=w;
end
