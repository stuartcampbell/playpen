...
!------------------
  ...
  function floatEqual (number1, number2 ) result (resultValue)
    ...
    ! test very small number1
    if ( abs(number1) < epsilon_sp .and.  abs(number1 - number2) < epsilon_sp ) then
      resultValue = .true.
    else 
      if ((abs(( number1 - number2)) / number1) < epsilon_sp ) then
        resultValue = .true.
      else
        resultValue = .false.
      end if
  ...
  function doublePrecisionEqual (number1, number2 ) result (resultValue)
    ...
    ! test very small number1
    if ( abs(number1) < epsilon_dp .and.  abs(number1 - number2) < epsilon_dp ) then
      resultValue = .true.
    else 
      if ((abs(( number1 - number2)) / number1) < epsilon_dp ) then
        resultValue = .true.
      else
        resultValue = .false.
      end if
