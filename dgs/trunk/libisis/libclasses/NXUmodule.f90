...
!
!$Id: NXUmodule.f90 1128 2007-05-17 15:58:57Z Freddie Akeroyd $
!------------------------------------------------------------------------------
...
! *** NeXus utility functions ***
   ...
   PUBLIC :: NXUpreparedata, NXUconfirmdata
! *** NeXus utility internal functions
   PRIVATE :: NXUsearchgroup
...
! *** NeXus generic interfaces ***
   INTERFACE NXUwritedata
       MODULE PROCEDURE NXUwritei4, NXUwriter4, NXUwriter8, NXUwritechar, &
                          ...
                          NXUwrite3Dr8array, NXUwrite4Di4array, &
                          NXUwrite4Dr4array, NXUwrite4Dr8array, &
                          NXUwrite1Dcarray
   END INTERFACE
   INTERFACE NXUreaddata
       MODULE PROCEDURE NXUreadi4, NXUreadr4, NXUreadr8, NXUreadchar, &
                          ...
                          NXUread4Di4array, NXUread4Dr4array, &
                          NXUread4Dr8array, &
                          NXUread1Dcarray
   END INTERFACE
...
!NXUwritechar writes a character data item
   FUNCTION NXUwritechar (file_id, data_name, data, units) RESULT (status)
      ...
      INTEGER :: status, slen
      slen = len_trim(data)
      if (slen == 0) slen = 1 ! cannot write a zero length string
...
!NXUread4Dr8array reads a 4D real*8 array
   FUNCTION NXUread4Dr8array (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
      ...
      CHARACTER(len=*), INTENT(out), OPTIONAL :: units
      INTEGER,          INTENT(in),  OPTIONAL :: data_start(:), data_size(:)
      INTEGER :: status, dimensions(NX_MAXRANK), data_shape(4)
      REAL, ALLOCATABLE :: buffer(:)
      if (present(units)) units = 'units'
      status = 0
   END FUNCTION NXUread4Dr8array

!NXUread4Dr8array reads a 4D real*8 array
   FUNCTION NXUread4Dr4array (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
      ...
      if (present(units)) units = 'units'
      status = 0
   END FUNCTION NXUread4Dr4array

!NXUread4Dr8array reads a 4D real*8 array
   FUNCTION NXUread4Di4array (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
      ...
      if (present(units)) units = 'units'
      status = 0
   END FUNCTION NXUread4Di4array

!NXUwrite3Dr4array writes 3D real*4 data
   FUNCTION NXUwrite4Dr4array (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
...
!NXUwrite3Dr4array writes 3D real*4 data
   FUNCTION NXUwrite4Dr8array (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
      ...
      status = NXUpreparedata (file_id, data_name, NX_FLOAT64, 4, shape(data))
      IF (status /= NX_OK) RETURN
      IF (PRESENT(units) .AND. NXUfindattr(file_id, "units") == NX_EOD) THEN
         status = NXputattr (file_id, "units", units)
         IF (status /= NX_OK) RETURN
...
!NXUwrite3Dr4array writes 3D real*4 data
   FUNCTION NXUwrite4Di4array (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
      ...
      INTEGER,          INTENT(in), OPTIONAL :: data_start(:), data_size(:)
      INTEGER :: status
      REAL(kind=NXi4), ALLOCATABLE :: buffer(:)
      status = 0
    end function
    

!------------------------------------------------------------------------------
   FUNCTION NXUwrite1Dcarray (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
      implicit none
      TYPE(NXhandle),   INTENT(inout) :: file_id
      CHARACTER(len=*), INTENT(in)    :: data_name
      ...
      CHARACTER(len=*), INTENT(in), OPTIONAL :: units
      INTEGER,          INTENT(in), OPTIONAL :: data_start(:), data_size(:)
      INTEGER :: status, n, i, j, data_dims(2)
      INTEGER(NXi1), ALLOCATABLE :: buffer(:)
      n = len(data(1))
      data_dims(1) = n
      data_dims(2) = size(data)
      status = NXUpreparedata (file_id, data_name, NX_INT8, 2, data_dims)
      IF (status /= NX_OK) RETURN
      IF (PRESENT(units) .AND. NXUfindattr(file_id, "units") == NX_EOD) THEN
         status = NXputattr (file_id, "units", units)
         IF (status /= NX_OK) RETURN
      END IF
      ALLOCATE (buffer(size(data)*n))
      do i=1,size(data)
         do j=1,n
            buffer((i-1)*n+j) = ichar(data(i)(j:j))
         enddo
      enddo
      IF (PRESENT(data_start) .AND. PRESENT(data_size)) THEN
         status = NXputslab (file_id, buffer, data_start, data_size)
   ...
   FUNCTION NXUread1Dcarray (file_id, data_name, data, units, data_start, &
                        data_size) RESULT (status)
      ...
      status = NXUconfirmdata (file_id, data_name, NX_INT8, 2, dimensions)
      IF (status /= NX_OK) RETURN
      IF (PRESENT(data_start) .AND. PRESENT(data_size)) THEN
         ALLOCATE (buffer(PRODUCT(data_size(1:2))))
         status = NXgetslab (file_id, buffer, data_start, data_size)
         IF (status == NX_OK) THEN
            ...
            do i=1,n
               data(i) = ' '
               do j=1,min(data_size(1),len(data(1)))
                  data(i)(j:j) = char(buffer((i-1)*data_size(1)+j))
               enddo
            enddo
         END IF
      ELSE
         ...
         IF (status == NX_OK) THEN
            ...
            do i=1,n
               data(i) = ' '
               do j=1,min(dimensions(1),len(data(1)))
                  data(i)(j:j) = char(buffer((i-1)*dimensions(1)+j))
               enddo
            enddo
         END IF
      END IF
