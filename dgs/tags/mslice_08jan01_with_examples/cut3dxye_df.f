...
C     SGI 64-bit platforms
      implicit NONE
      integer plhs(*), prhs(*), nrhs, nlhs, temp
...
C declare local variables to deal with pointers to variables passed by/to MATLAB
      ...
      integer x_pr, intensity_pr, error_int_pr
...
C     Check for proper number of MATLAB input and output arguments 
     ...
     c ('Six inputs (vx,vy,vz,pixel_int,pixel_err,grid) required.')
      elseif (nlhs .ne. 5) then
         call mexErrMsgTxt
     c  ('Five outputs (x,intensity,err_int,vy,vz) 
     c required.')
      end if
...
C     Create matrices for the return arguments (operating workspace for the subroutine cut3dxye_df)
      plhs(1)	    =mxCreateFull(1,n,0) ! these are matlab pointers
...
C     Call the computational subroutine cut3dxye_df
      call cut3dxye_df(%val(vx_pr),%val(vy_pr),%val(vz_pr),
...
C     Reduce size of output arrays x,intensity,error_int to useful data only  
      if (m .lt. n) then
...
C
C ===============================================================================
C actual FORTRAN code for the cut3dxye_df algorithm
C distribute all data points into bins, then take average over each bin
C =============================================================================== 
      subroutine cut3dxye_df(vx,vy,vz,
